...
def test_list_pets():
    ...
    mock_connection.session.all.assert_called_once()  # foi usado all
    mock_connection.session.filter.assert_not_called()  # não foi usado filter
...
def test_list_pets_no_result():
    ...
    mock_connection.session.query.assert_called_once_with(PetsTable)
    mock_connection.session.all.assert_not_called()  # não foi usado all
    mock_connection.session.filter.assert_not_called()  # não foi usado filter
